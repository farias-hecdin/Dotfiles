    ...
    config = function()
      ...
      hipatterns.setup({
        highlighters = {
          ...
          bug      = { pattern = {"()BUG:()", "()FIX:()", "()FIXME:()", "()ISSUE:()", "()DEPRECATED()"}, group = "MiniHipatternsFixme" },
          hack     = { pattern = {"()HACK:()", "()WARN:()", "()WARNING:()"}, group = "MiniHipatternsHack" },
          test     = { pattern = {"()TEST:()", "()TODO:()"}, group = "MiniHipatternsTodo" },
          info     = { pattern = {"()INFO:()", "()NOTE:()"}, group = "MiniHipatternsNote" },
          optimize = { pattern = {"()OPTIMIZE:()","()PERF:()"}, group = "MiniHipatternsPerf" },

          -- Extras (ex: * @param - !THIS: - STEP 01: #1000:
          docs     = { pattern = {"%s()@%l+()"}, group = "MiniHipatternsDocs" },
          step     = { pattern = {"()STEP %d+:()", "()#%d+:()"}, group = "MiniHipatternsStep" },
